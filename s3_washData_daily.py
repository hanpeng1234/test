    ...
    for row in databases:
        ...
        if databaseName != "" and  databaseName in dbmap :
            ...
            for row in tablelist:
                ...
                hissql="select etldate,0 as etlindex, "+colum+" year,month,day"+" ,'insert' as kind from "+dbmap[databaseName]+"_etl_s3"+"."+tableName
                ...
                spark.catalog.refreshTable(dbmap[databaseName]+"_etl_s3"+"."+tableName)
...
# # # #                  --- newdata.write.format("orc").mode("overwrite").orc("s3://rupiahplus-data-warehouse/stream/datademo/")
# # # #                  --- newdata.write.format("org.apache.spark.sql.json").mode("overwrite").save("hdfs:///table_s3_etl_tmp/")
                ...
                # add partitions
                partitionsql="ALTER TABLE "+dbmap[databaseName]+"_etl_s3"+"."+tableName+"  ADD IF NOT EXISTS PARTITION (year=" +partitionlist[0] + ",month=" + partitionlist[1] + ",day=" + partitionlist[2] + ")"
                spark.sql(partitionsql)
                spark.catalog.dropTempView("tmp")
#     date=datetime.now()
#     dicts = [{'message':'end','date':date}]
#     dataframe = spark.createDataFrame(dicts)
#     dataframe.write.format("org.apache.spark.sql.json").mode("overwrite").save("hdfs:///table_s3_etl_tmp/")